...
function _smoothScroll({
  ...
  if (updateHistory && window.history.pushState && location.hash !== hash)
  ...
  install(vue, config) {
    ...
    vue.directive("smooth-scroll", {
      [isVue3 ? "mounted" : "inserted"](el, binding, vnode) {
        ...
        if (typeof window !== "object" || window.pageYOffset === undefined)
          return;

        let { duration, offset, container, updateHistory, easingFunction } =
          binding.value || {};
